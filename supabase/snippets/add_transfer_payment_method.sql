-- Añadir 'transfer' como método de pago válido
-- 1. Actualizar el constraint de orders
ALTER TABLE public.orders DROP CONSTRAINT IF EXISTS orders_payment_method_check;
ALTER TABLE public.orders ADD CONSTRAINT orders_payment_method_check
  CHECK (payment_method IN ('card', 'cash', 'mercado_pago', 'transfer'));

-- 2. Actualizar el constraint de businesses
ALTER TABLE public.businesses DROP CONSTRAINT IF EXISTS businesses_payment_methods_valid;
ALTER TABLE public.businesses ADD CONSTRAINT businesses_payment_methods_valid
  CHECK (accepted_payment_methods <@ ARRAY['cash', 'mercado_pago', 'transfer']);

-- 3. Actualizar la función create_order_with_items para que soporte transfer
...
BEGIN
  ...
  END IF;
  -- AQUI ESTA LA MODIFICACION: Añadido 'transfer'
  IF p_payment_method NOT IN ('card', 'cash', 'mercado_pago', 'transfer') THEN
